...
TW3DFramebuffer* TW3DResourceManager::CreateFramebuffer(TWT::String Name, TWT::uint2 Size, TW3DFramebufferType Type) {
	return new TW3DFramebuffer(Name, this, Size, Type);
}
...
TW3DRenderTarget* TW3DResourceManager::CreateRenderTarget(TWT::String Name, ID3D12Resource* Buffer) {
	auto* rtv = new TW3DRenderTarget(Name, device, rtv_descriptor_heap, srv_descriptor_heap);
	rtv->Create(Buffer);
	return rtv;
}

TW3DRenderTarget* TW3DResourceManager::CreateRenderTarget(TWT::String Name, TWT::uint2 Size, DXGI_FORMAT Format, TWT::float4 ClearValue) {
	auto* rtv = new TW3DRenderTarget(Name, device, rtv_descriptor_heap, srv_descriptor_heap, Format, ClearValue);
	rtv->Create(Size);
	return rtv;
}

TW3DRenderTarget* TW3DResourceManager::CreateRenderTargetCube(TWT::String Name, TWT::uint Size, DXGI_FORMAT Format, TWT::float4 ClearValue) {
	auto* rtv = new TW3DRenderTarget(Name, device, rtv_descriptor_heap, srv_descriptor_heap, Format, ClearValue);
	rtv->CreateCube(Size);
	return rtv;
}
...
TW3DBuffer* TW3DResourceManager::CreateBuffer(TWT::String const& Name, TWT::uint ElementCount, TWT::uint ElementSizeInBytes,
	...
	auto* buffer = new TW3DBuffer(Name, device, temp_gcl, UpdateOptimized, ElementSizeInBytes, UAV, srv_descriptor_heap);
...
TW3DVertexBuffer* TW3DResourceManager::CreateVertexBuffer(TWT::String Name, TWT::uint VertexCount, TWT::uint SingleVertexSizeInBytes, bool OptimizeForUpdating) {
	return new TW3DVertexBuffer(Name, device, temp_gcl, OptimizeForUpdating, VertexCount, SingleVertexSizeInBytes);
}
...
TW3DConstantBuffer* TW3DResourceManager::CreateConstantBuffer(TWT::String const& Name, TWT::uint ElementCount, TWT::uint ElementSizeInBytes) {
	auto buffer = new TW3DConstantBuffer(Name, device, ElementCount, ElementSizeInBytes);
	TWU::DXSetName(buffer->Native, Name);
	return buffer;
}

TW3DTexture* TW3DResourceManager::CreateDepthStencilTexture(TWT::String Name, TWT::uint2 Size) {
	auto* texture = new TW3DTexture(Name, device, temp_gcl, DXGI_FORMAT_D32_FLOAT, nullptr, nullptr, dsv_descriptor_heap);
	texture->CreateDepthStencil(Size);
	return texture;
}

TW3DTexture* TW3DResourceManager::CreateDepthStencilCubeTexture(TWT::String Name, TWT::uint Size, D3D12_RESOURCE_FLAGS Flags, D3D12_DSV_FLAGS DSVFlags) {
	auto* texture = new TW3DTexture(Name, device, temp_gcl, DXGI_FORMAT_D32_FLOAT, nullptr, nullptr, dsv_descriptor_heap);
...
TW3DTexture* TW3DResourceManager::CreateTexture2D(TWT::String Name, TWT::uint2 Size, DXGI_FORMAT Format, bool UAV) {
	auto* texture = new TW3DTexture(Name, device, temp_gcl, Format, srv_descriptor_heap, UAV ? uav_descriptor_heap : nullptr, nullptr);
	texture->Create2D(Size);
	return texture;
}

TW3DTexture* TW3DResourceManager::CreateTextureArray2D(TWT::String Name, TWT::uint2 Size, TWT::uint Depth, DXGI_FORMAT Format, bool UAV) {
	auto* texture = new TW3DTexture(Name, device, temp_gcl, Format, srv_descriptor_heap, UAV ? uav_descriptor_heap : nullptr, nullptr);
...
TW3DRenderTarget* TW3DResourceManager::RequestRenderTarget(TWT::String const& Name, ID3D12Resource* Buffer) {
	...
	if (!rt) {
		rt = CreateRenderTarget(Name, Buffer);
...
TW3DRenderTarget* TW3DResourceManager::RequestRenderTarget(TWT::String const& Name, TWT::uint2 Size, DXGI_FORMAT Format, TWT::float4 ClearValue) {
	...
	if (!rt) {
		rt = CreateRenderTarget(Name, Size, Format, ClearValue);
		TWU::DXSetName(rt->Native, Name);
	}
...
TW3DRenderTarget* TW3DResourceManager::RequestRenderTargetCube(TWT::String const& Name, TWT::uint Size, DXGI_FORMAT Format, TWT::float4 ClearValue) {
	...
	if (!rt) {
		rt = CreateRenderTargetCube(Name, Size, Format, ClearValue);
		TWU::DXSetName(rt->Native, Name);
	}
...
TW3DTexture* TW3DResourceManager::RequestTexture2D(TWT::String const& Name, TWT::uint2 Size, DXGI_FORMAT Format, bool UAV) {
	...
	if (!texture) {
		texture = CreateTexture2D(Name, Size, Format, UAV);
...
TW3DTexture* TW3DResourceManager::RequestTextureArray2D(TWT::String const& Name, TWT::uint2 Size, TWT::uint Depth, DXGI_FORMAT Format, bool UAV) {
	...
	if (!texture) {
		texture = CreateTextureArray2D(Name, Size, Depth, Format, UAV);
		TWU::DXSetName(texture->Native, Name);
	}
...
TW3DTexture* TW3DResourceManager::RequestDepthStencilTexture(TWT::String const& Name, TWT::uint2 Size) {
	...
	if (!texture) {
		texture = CreateDepthStencilTexture(Name, Size);
		TWU::DXSetName(texture->Native, Name);
	}
...
TW3DTexture* TW3DResourceManager::RequestDepthStencilCubeTexture(TWT::String const& Name, TWT::uint Size, D3D12_RESOURCE_FLAGS Flags, D3D12_DSV_FLAGS DSVFlags) {
	...
	if (!texture) {
		texture = CreateDepthStencilCubeTexture(Name, Size, Flags, DSVFlags);
		TWU::DXSetName(texture->Native, Name);
	}
...
TW3DVertexBuffer* TW3DResourceManager::RequestVertexBuffer(TWT::String const& Name, TWT::uint VertexCount, TWT::uint SingleVertexSizeInBytes, bool OptimizeForUpdating) {
	...
	if (!buffer) {
		buffer = CreateVertexBuffer(Name, VertexCount, SingleVertexSizeInBytes, OptimizeForUpdating);
		TWU::DXSetName(buffer->Native, Name);
	}
...
TW3DFramebuffer* TW3DResourceManager::RequestFramebuffer(TWT::String const& Name, TWT::uint2 Size, TW3DFramebufferType Type) {
	auto& buffer = framebuffers[Name];

	if (!buffer)
		buffer = CreateFramebuffer(Name, Size, Type);
